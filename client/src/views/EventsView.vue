<template>
  <div v-if="!isLoading">
    <v-row class="bg-primaryWhite pt-6 mx-3" align="center" no-gutters v-if="eventStore.events.length != 0">
      <v-text-field prepend-inner-icon="mdi-magnify" hide-details variant="outlined" placeholder="Search Events"
        class="mr-3" clearable v-model="searchQuery" />
      <v-btn v-if="tempIsTeamAdmin" size="x-large" density="compact" variant="flat" icon="mdi-plus"
    ...
    <div id="cards-container" class="bg-primaryWhite pt-4">
      ...
      <div v-if="filteredEventsList.length == 0" class="mt-6 mx-3 text-center">
        ...
        <v-btn v-if="tempIsTeamAdmin" size="x-large" class="bg-primaryRed text-primaryWhite mt-3"
          @click="isAddingEvent = true">ADD EVENT
        </v-btn>
      </div>
    </div>
  </div>
