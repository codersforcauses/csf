...
const capitalize = (words: string) => words.split(' ').map(word => word[0].toUpperCase() + word.substring(1)).join(' ')
...
<template>
    ...
    <v-btn class="bg-primaryRed" @click="display = true">
        <PopupDialog v-model="display" :title="'Confirm ' + capitalize(`${props.object} ${props.action}`)"
            :text="`Are you sure you wish to ${props.action} this ${props.object}?`" :submit-text="capitalize(props.action)"
