...
class MileageTests(APITestCase):
    def _get_token(self):
        ...
        get_token_body = {"username": USERNAME, "password": PASSWORD}
    ...
    def test_get_mileage(self):
        response = self.client.get(
            reverse("mileage:get-mileage"), {"user": self.user.id}
        )
    ...
    def test_total_km(self):
        response = self.client.post(
            ...
            {"user": self.user.id, "kilometres": 200.0},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        response = self.client.get(
            reverse("mileage:get-mileage"), {"user": self.user.id, "sum": True}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_get_challenge_mileages(self):
        ...
        response = self.client.get(
            reverse("mileage:get-mileage"), {"challenge": True, "user": self.user.id}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_leaderboard(self):
        ...
        user1 = User.objects.create(
            username="user1", email="user1@eample.com", team_id=team1
        )
        user2 = User.objects.create(
            username="user2", email="user2@eample.com", team_id=team1
        )
        user3 = User.objects.create(
            username="user3", email="user3@eample.com", team_id=team2
        )
        ...
        self.assertEquals(self.user.team_id, None)
        # self.assertEquals(self.user., team1)
        url = reverse("mileage:post-mileage")
        ...
        self.assertEquals(
            response.data["team"],
            {"name": "team2", "bio": "we are team 2", "rank": 2, "total_mileage": 6.0},
        )
